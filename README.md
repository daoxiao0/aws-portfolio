...
S3 + CloudFront → + Route53/ACM   → + Cognito/Lambda/DynamoDB → + CloudWatch/X-Ray/SNS               → + ECS Fargate/ALB/RDS
(Static)           (Custom Domain)    (Serverless)                (Observability — instruments Phase 3)   (Containers — standalone demo, no NAT Gateway)
```
...
Phase 01           Phase 02               Phase 03                    Phase 04                          Phase 05
S3 + CloudFront → + Route53/ACM       → + Cognito/Lambda/DynamoDB → + CloudWatch/X-Ray/SNS           → + ECS Fargate/ALB/RDS
（静的配信）        （カスタムドメイン）    （サーバーレス）              （オブザーバビリティ — Phase 3を計装）  （コンテナ — 単体デモ・NAT Gatewayなし）
```
